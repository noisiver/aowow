    ...
    protected function generateContent()
    {
        ...
        $getItemSource = function ($itemId, $method = 0) use (&$mapNPCs, &$mapGOs)
        {
            ...
            $vendors = DB::World()->selectCol('
                SELECT nv.entry FROM npc_vendor nv WHERE nv.item = ?d AND ?d BETWEEN MinPatch AND MaxPatch UNION
                SELECT c.id1 FROM game_event_npc_vendor genv JOIN creature c ON c.guid = genv.guid WHERE genv.item = ?d AND ?d BETWEEN c.MinPatch AND c.MaxPatch',
