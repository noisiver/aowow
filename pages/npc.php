...
class NpcPage extends GenericPage
{
    ...
    protected function generateContent()
    {
        ...
        if ($this->subject->getField('aiName') == 'SmartAI')
        {
            ...
            if (!$sai->prepare())                           // no smartAI found .. check per guid
            {
                ...
                $guids = DB::World()->selectCol('SELECT guid FROM creature WHERE id1 = ?d AND ?d BETWEEN MinPatch AND MaxPatch', $this->typeId, PROGRESSION_PATCH);
        ...
        if ($this->subject->getField('npcflag') & NPC_FLAG_TRAINER)
        {
            $teachQuery = '
                ...
                WHERE     t1.ID = ?d AND ?d BETWEEN t1.MinPatch AND t1.MaxPatch AND ?d BETWEEN t2.MinPatch AND t2.MaxPatch
            ';
        ...
        // tab: sells
        if ($sells = DB::World()->selectCol('SELECT item FROM npc_vendor nv WHERE entry = ?d AND ?d BETWEEN MinPatch AND MaxPatch UNION SELECT item FROM game_event_npc_vendor genv JOIN creature c ON genv.guid = c.guid WHERE c.id1 = ?d AND ?d BETWEEN c.MinPatch AND c.MaxPatch', $this->typeId, PROGRESSION_PATCH, $this->typeId, PROGRESSION_PATCH))
        {
    ...
    */
        ...
        $lootGOs = DB::World()->select('select SourceEntry, ConditionValue1, ConditionValue2 from conditions where SourceTypeOrReferenceId = 1 and SourceGroup = ?d and ConditionTypeOrReference = ?d AND ?d BETWEEN MinPatch AND MaxPatch', $this->typeId, CND_SKILL, PROGRESSION_PATCH);
